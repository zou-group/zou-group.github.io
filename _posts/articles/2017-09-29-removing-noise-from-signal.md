...
Perhaps the best-known example of this is the **application of the Fourier Transform to audio signal**. If you take a recording of someone speaking, it's going to have all sorts of noise in it because of background sounds. This may not be clear from the raw signal. But if you break your signal [down by frequencies)](https://en.wikipedia.org/wiki/Fourier_transform), you'll see that most of the audio signal lies in just a few frequencies. The noise, because it's random, will still be spread out across all of the frequencies. This means that we can *filter* out our noise by retaining only the frequencies with lots of signal and removing all other frequencies.
